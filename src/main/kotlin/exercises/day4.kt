...
fun validatePassword(passport: String, validator: (String, String) -> Boolean) = passport
    .split("""[\s(\r\n)]""".toRegex())
    .filter { it.isNotBlank() }
    .count {
      it.split(':')
      .let { (entry, value) -> validator(entry, value) }
    } == 7

fun simpleValidator (entry: String, value: String) = entry.matches(
  """(byr|iyr|eyr|hgt|hcl|ecl|pid)""".toRegex()
)

fun advancedValidator(entry: String, value: String) = when (entry){
